   ...
   it.only("Verifying the Bumper Coupon is activation after First Order",function() {
           ...
           // cy.get('.css-1jt1w2w').click()
            ...
            //  cy.get('.css-1eu3kd4 > .css-1udy5o0').then(($element) =>{
              cy.wait(2000)
              cy.screenshot();
              const text2 = cy.get('.css-1eu3kd4 > .css-1xu1cgw > .css-1b1pxu2 > .css-1bko2et > .css-1kvwatx');
           
              if (text2.contains("BUMPER")) {
                cy.log("Bumper is Activated") 
                cy.log(text2);
                cy.wait(14000)
                cy.get('.css-ar3zkj').click() // Start Shopping Button 
                cy.get('.css-16h0kxh > :nth-child(3)').click() // click from Best Sellers Page 
                cy.wait(2000)

                const text3 = cy.get('.css-17rod50');
                ...
                const Bumper_offer_applied = cy.get('.css-7yd8vv > :nth-child(5)');
